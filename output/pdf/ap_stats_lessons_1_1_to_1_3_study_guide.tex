...
0--15 min & Memorize the vocabulary and denominator rules on pages 2--5.\\
15--30 min & Redo the two class-data worked examples without looking at the blue solutions.\\
30--50 min & Complete the mixed practice set on pages 7--9.\\
...
\[
\text{height}=\text{conditional proportion},\qquad
\text{width}=\text{group's share of the sample},\qquad
...
\begin{center}
\begin{tabularx}{\textwidth}{>{\bfseries\color{Navy}}p{1.7in} Y}
\toprule
When you see... & Think...\\
\midrule
...
\begin{formulabox}{The four formulas worth memorizing}
\begin{align*}
\text{relative frequency} &= \frac{\text{category count}}{\text{total count}},\\
\text{joint relative frequency} &= \frac{\text{one interior cell}}{\text{grand total}},\\
\text{conditional relative frequency} &= \frac{\text{one interior cell}}{\text{total of the given group}},\\
\text{count from a percent} &= (\text{decimal proportion})(\text{group size}).
\end{align*}
\end{formulabox}
